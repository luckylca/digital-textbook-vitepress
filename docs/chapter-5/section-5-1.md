...
异步电机的功率范围很宽，可以从 kW 级到 $10^3\,\mathrm{kW}$ 级。它常见的优点和不足如下：
...
异步电机不是转子直接接电源后自己转起来，而是定子先建立旋转磁场，转子再通过电磁感应获得电流，最后由感应电流和旋转磁场相互作用产生转矩。

<StepExplain title="基本工作过程" :steps="principleSteps" />

<WorkingPrincipleFlow
  title="工作原理交互演示"
  subtitle="点击右侧环节，观察定子、气隙旋转磁场、转子导体和电磁转矩的对应关系。"
  :steps="section51FlowSteps"
/>

<KnowledgeCard title="核心过程" type="summary">
...
| 运行状态 | 条件 | 转矩性质 | 能量转换 |
| --- | --- | --- | --- |
| 电动机状态 | $0 < n < n_1$，$0 < s < 1$ | $T_{em}$ 与 $n$ 同向，为驱动转矩 | 电网电能 → 机械动能 |
| 发电机状态 | $n > n_1$，$s < 0$ | $T_{em}$ 与 $n$ 反向，为制动转矩 | 机械动能 → 电网电能 |
| 电磁制动状态 | $n < 0$，$s > 1$ | $T_{em}$ 与 $n$ 反向，为制动转矩 | 机械动能 + 电网电能 → 内部损耗 |
...
异步电动机铭牌上常见的额定值包括：额定功率 $P_N$、额定电压 $U_N$、额定电流 $I_N$、额定频率 $f_N$ 和额定转速 $n_N$。
...
额定转速 $n_N$ 是电机在额定电压、额定频率下输出额定功率时的转子转速，单位为 r/min。

## 例题讲解

<ExampleBlock title="例题 1：同步转速与转差率计算">

**题目**：一台三相异步电动机接在 50 Hz 电网上，绕组极对数 $p = 2$，额定运行时转子转速 $n = 1440\,\mathrm{r/min}$。求同步转速 $n_1$ 和转差率 $s$。
...
$$
n_1 = \frac{60 \times 50}{2} = 1500\,\mathrm{r/min}
$$
...
**答案**：同步转速 $n_1 = 1500\,\mathrm{r/min}$；转差率 $s = 0.04$，也就是 4%。
...
**题目**：某异步电机的同步转速 $n_1 = 1500\,\mathrm{r/min}$。判断以下三种转速分别对应什么运行状态：

A. $n = 1440\,\mathrm{r/min}$  
B. $n = 1550\,\mathrm{r/min}$  
C. $n = -200\,\mathrm{r/min}$

**解题思路**：先根据 $s = (n_1 - n) / n_1$ 计算转差率，再根据 $s$ 的范围判断状态。
...
**易错提醒**：不要只看转速大小，要结合同步转速 $n_1$ 和转差率 $s$ 判断。

</ExampleBlock>

<ExampleBlock title="例题 3：额定功率计算">

**题目**：一台三相异步电动机额定线电压 $U_N = 380\,\mathrm{V}$，额定线电流 $I_N = 5\,\mathrm{A}$，额定效率 $\eta_N = 0.82$，额定功率因数 $\cos\varphi_N = 0.80$。求额定输出功率 $P_N$。
...
$$
P_N \approx 2159\,\mathrm{W} \approx 2.16\,\mathrm{kW}
$$

**答案**：额定输出功率 $P_N$ 约为 2159 W，也就是 2.16 kW。

**易错提醒**：这里的 $U_N$ 和 $I_N$ 是线电压、线电流，不需要因为 Y 接或 Δ 接再换成相电压、相电流。

</ExampleBlock>

## 互动演示：同步转速与转差率

下面的演示可以直接拖动参数，也可以点击状态示例。观察重点是：$f_1$ 越大，$n_1$ 越大；$p$ 越大，$n_1$ 越小；$n$ 和 $n_1$ 的相对关系会改变切割磁感线强弱和运行状态。
